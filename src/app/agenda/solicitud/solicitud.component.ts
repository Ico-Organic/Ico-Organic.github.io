...
export class SolicitudComponent implements OnInit {
    ...
    if(this.idUser!){
      ...
      if(this.IsMasCoun == true){
       ...
       if(this.IsMasString != 'Productor'){
        ...
        this.parcela = []
        let id =    this._AuthGuard.IsIDS()
        this.registroForm.patchValue({solicitorTo: id}) 
        this.chooseUser(this.idUser)
       }
  ...
  SelectDate(event: any) {
    var newevent = moment(moment(new Date(event)).add(24, 'hours').format('LLL')).format("yyyy-MM-DD");
 ...
 /*Existe() {
    if (this.id!) {
      ...
      if (this.IsMasCoun == false) {
        ...
        this.registroForm.patchValue({ solicitorTo: this.id })
        console.log(this.registroForm.value);
        console.log(this.parcelas);

        this.parcelas.filter((element: any) => {
          if (element.productor == this.registroForm.value.solicitorTo) {
            console.log("lcoooooooooooooo");
              console.log(element);
              
            this.parcelaHis.push(element)
          }
        });
